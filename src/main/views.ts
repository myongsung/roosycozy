...
<div class="actions">
          ...
          ${H.iconBtn('🧪', 'load-sample', '샘플 불러오기(현재 데이터 덮어쓰기)')}
...
function renderRecordsMain() {
  ...
  return `
    ...
    <div class="helperBox aiHelp"><b>팁:</b> 메모를 쌓아두면, 다음 탭에서 <b>AI가 메모 묶음으로 자동 모아</b>줘요.</div>
  `;
}
    ...
    return `
      <div class="sectionTitle">
        <div>
          ...
          <div class="muted">요약을 입력하면 AI가 관련 메모를 자동 선별해 메모 묶음 타임라인으로 모아줘요.</div>
        </div>
      </div>
      ${renderDefenseIntro()}
...
function renderCaseCreateModal() {
  ...
  return H.modal(
    'caseCreateModal',
    H.modalHead('스마트 메모 모으기', '요약을 입력하면 AI가 관련 메모를 모아 타임라인으로 보여줘요.', H.btn('닫기', 'close-case-create')),
    `
...
function renderDefenseIntro() {
  return `
    <div class="defenseIntro">
      <div class="defenseIntroTitle">메모가 쌓이면, AI가 <b>메모 묶음 타임라인</b>으로 모아줘요.</div>
